...
(async()=>{
 ...
 for(const [name,width,height] of [['desktop',1440,1000],['mobile',390,844]]){
  ...
  await page.locator('[data-tab="evidence"]').click();assert.equal(await page.locator('.export-list [data-export]').count(),10);const validationItems=await page.locator('.validation-plan li').allTextContents();assert(validationItems.length>0,'Validation plan must contain a recorded next step');assert(validationItems.every(item=>item.trim().length>0&&!item.includes('No validation plan supplied')),'Validation steps must contain actionable recorded content');
