...
def figures(root):
    ...
    for i, row in enumerate(binding):
        ...
        ax.scatter([i + (j - 1) * .055 for j in range(len(values))], values, color=["#bc4841", "#3275aa", "#568e81"][i], s=65)
    ...
    ax.set_title("WT residue–ligand contact frequency", loc="left", fontweight="bold", pad=18)
    fig.colorbar(im, ax=ax, label="Observed seed fraction (n = 3)", shrink=.55)
    fig.text(.06, .01, "Heavy-atom proximity ≤5 Å. A contact is a pose hypothesis, not binding evidence.", fontsize=8)
    save(fig, "residue-contact-matrix", data["contact_definitions"])
    comparisons = data["structural_comparisons"]
    groups = [("WT seed comparisons", [r for r in comparisons if r.get("subject") == "arabidopsis_wt" and "query_tm_score" in r]),
    ...
    fig.suptitle("Repeatability and homolog accuracy have different scopes", x=.08, ha="left", fontweight="bold")
    fig.tight_layout(rect=[0, .04, 1, .94])
    fig.text(.08, .005, "Paired comparisons are correlated. No same-target experimental accuracy or functional calibration is established.", fontsize=9)
