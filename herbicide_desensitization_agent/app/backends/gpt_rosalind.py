...
from .interfaces import RosalindReasoningBackend
from ..agents.prompts import CANDIDATE_REVIEW_PROMPT, DOMAIN_JUDGE_PROMPT
from ..schemas.models import EvaluationPacket


class GPTRosalindJSONBackend(RosalindReasoningBackend):
    """Transport-injected GPT-Rosalind adapter with no persisted credentials."""
    ...
    def review(self, facts: list[str], predictions: list[str]) -> dict:
        ...
        for key in ("assumptions", "uncertainty", "recommendation"):
            if key not in result:
                raise ValueError(f"GPT-Rosalind review response is missing {key}")
        return result
